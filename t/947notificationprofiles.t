#!/usr/bin/perl
# Tests for Opsview::ResultSet::Notificationprofiles
...
my $rs = $schema->resultset( "Notificationprofiles" );
...
is( $schema->resultset("Metadata")->find("uncommitted")->value,
    1, "Committed" );
@nps = $nonadmin->notificationprofiles;
...
is(
    $temp_profile->host_notification_options,
    "n", "Got an 'n' when no host options set"
);
is(
    $temp_profile->service_notification_options,
    "n", "Got an 'n' when no service options set"
);

my ( @hgids, @sgids, @keyids );

my $restricted =
  $schema->resultset("Contacts")->find( { name => "viewsomechangenone" } );
is(
    $restricted->notificationprofiles->count,
    1, "Only 1 notification profile"
);
...
is(
    $restricted->notificationprofiles->count,
    2, "New notification profile created"
);
...
like(
    $@,
    qr/Failed constraint on name for 'bad%%chars' with regex/,
    "Constraints on name"
);
    ...
    $rs->synchronise(
        {
            contactid                    => 1,
            name                         => "Adminprofile",
            host_notification_options    => "fsd",
            service_notification_options => "blah"
        }
    );
};
is(
    $@,
    "host_notification_options: Invalid; service_notification_options: Invalid\n",
